...
public class ChessGame {
  ...
  public boolean isInCheck(TeamColor teamColor) {
    ...
    return canAnyPieceCaptureKing(teamColor);
  }

  private boolean canAnyPieceCaptureKing(TeamColor teamColor) {
    for (int r=1; r < 9; r++) {
      for (int c=1; c < 9; c++) {
        ChessPiece currentPiece=board.getPiece(new ChessPosition(r, c));
        if (isOpponentPiece(currentPiece, teamColor) &&
                canPieceCaptureKing(currentPiece, new ChessPosition(r, c))) {
          return true;
        }
      }
    }
  ...
  private boolean canPieceCaptureKing(ChessPiece piece, ChessPosition position) {
    // Get all valid moves for the piece and check if any can capture the King
    Collection<ChessMove> possibleMoves=validMoves(position);
    return anyMoveCapturesKing(possibleMoves);
  }

  private boolean anyMoveCapturesKing(Collection<ChessMove> possibleMoves) {
    // Check if any move leads to capturing the King
